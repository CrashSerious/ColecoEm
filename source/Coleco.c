...
char *CartName   = "/dev_hdd0/game/PS3ADAMEm/USRDIR/CART.rom";    /* Cartridge ROM file                     */
char *OS7File    = "/dev_hdd0/game/PS3ADAMEm/USRDIR/OS7.rom";     /* ColecoVision ROM file                  */
char *EOSFile    = "/dev_hdd0/game/PS3ADAMEm/USRDIR/EOS.rom";     /* EOS ROM file                           */
char *WPFile     = "/dev_hdd0/game/PS3ADAMEm/USRDIR/WP.rom";      /* SmartWriter ROM file                   */
char *ExpRomFile = "/dev_hdd0/game/PS3ADAMEm/USRDIR/EXP.rom";     /* Expansion ROM file                     */
char *HardDiskFile = "/dev_hdd0/game/PS3ADAMEm/USRDIR/hdisk.img"; /* Hard Disk Image file */
...
byte *ROM,*CART,*OS7,*EOS,*WP,*EXPROM;    /* ROM pointers                           */
...
//static FILE *HardDisk;			  /* Hard disk stream						*/
...
/* Hard Disk Emulation */
//static byte SecCnt,SecNum;
//static byte CylLow, CylHigh;
//static byte SDHReg;
//static byte ErrReg;
//static byte StatReg = 0x48;
//static byte CmdReg;
static byte AltStatReg;
//static byte FixedCtlReg;
static byte DigInReg;
//static byte DataLow,DataHigh;
HarddiskIde *HardDisk;
...
static void Out02 (byte Val)
{
//	printf("SecCnt %d\n",Val);
//	SecCnt = Val;
	harddiskIdeWriteRegister(HardDisk, 2,Val);
}

static void Out03 (byte Val)
{
//	printf("SecNum %d\n",Val);
//	SecNum = Val;
	harddiskIdeWriteRegister(HardDisk, 3,Val);
}

static void Out04 (byte Val)
{
//	printf("CylLow %d\n",Val);
//	CylLow = Val;
	harddiskIdeWriteRegister(HardDisk, 4,Val);
}

static void Out05 (byte Val)
{
//	printf("CylHigh %d\n",Val);
//	CylHigh = Val;
	harddiskIdeWriteRegister(HardDisk, 5,Val);
}

static void Out06 (byte Val)
{
//	printf("SDHReg %d\n",Val);
//	SDHReg = Val;
	harddiskIdeWriteRegister(HardDisk, 6,Val-160);
}

static void Out07 (byte Val)
{
//	printf("CmdReg %d\n",Val);
//	CmdReg = Val;
	harddiskIdeWriteRegister(HardDisk, 7,Val);
	
}

static void Out58 (byte Val)
{
//	printf("DataLow %d\n",Val);
//	DataLow = Val;
	HardDiskWriteData |= Val;
	harddiskIdeWrite(HardDisk,HardDiskWriteData);
	
}

static void Out59 (byte Val)
{
//	printf("DataHigh %d\n",Val);
//	DataHigh = Val;
...
OutPortFn OutPort[256]=
{
	...
	OutNo,OutNo,OutNo,OutNo,Out0C,Out0D,Out0E,Out0F,
	OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,
	OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,
	OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,
	OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,
	OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,
	...
	OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,
	OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,
	Out58,Out59,Out5A,OutNo,OutNo,OutNo,OutNo,OutNo,
	OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,
	OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,
	OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,
	OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,OutNo,
...
static byte Inp01 (void)
{
//	printf("Got ErrReg\n");
//	return 0;
	//  return ErrReg;
	return harddiskIdeReadRegister(HardDisk, 1);
}

static byte Inp02 (void)
{
//	printf("SecCnt %d\n",SecCnt);
//	return SecCnt;
	return harddiskIdeReadRegister(HardDisk, 2);
}

static byte Inp03 (void)
{
//	printf("SecNum %d\n",SecNum);
//	return SecNum;
	return harddiskIdeReadRegister(HardDisk, 3);
}

static byte Inp04 (void)
{
//	printf("CylLow %d\n",CylLow);
//	return CylLow;
	return harddiskIdeReadRegister(HardDisk, 4);
}

static byte Inp05 (void)
{
//	printf("CylHigh %d\n",CylHigh);
//	return CylHigh;
	return harddiskIdeReadRegister(HardDisk, 5);

}

static byte Inp06 (void)
{
//	printf("SDHReg %d\n",SDHReg);
//	return SDHReg;
	return harddiskIdeReadRegister(HardDisk, 6);
}

static byte Inp07 (void)
{
//	if (StatReg == 0x48)
//		StatReg = 0x5a;
//	else
...
//	printf("StatReg %x\n",StatReg);
//	return StatReg;
	return harddiskIdeReadRegister(HardDisk, 7);
}
...
static byte Inp58 (void)
{
//	printf("DataLow\n");
//	return DataLow;
	return (byte)harddiskIdePeek(HardDisk);
}

static byte Inp59 (void)
{
//	printf("DataHigh\n");
//	return DataHigh;
	return harddiskIdeRead(HardDisk) >> 8;
}
...
InPortFn InPort[256]=
{
	...
	Inp08,Inp09,Inp0A,Inp0B,InpNo,InpNo,InpNo,InpNo,
	...
	InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,
	InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,
	Inp58,Inp59,Inp5A,Inp5B,InpNo,InpNo,InpNo,InpNo,
	InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,
	InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,
	InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,
	InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,InpNo,
...
static int GetSnapshotParams (FILE *f)
{
 ...
 buf[15]='\0';
 if (strcmp(buf,"ADAMEm snapshot")) return -2;  /* Not a snapshot file      */
...
static int _SaveSnapshotFile (FILE *f)
{
 ...
 if (fwrite(buf,1,1,f)!=1) return -2;
 memset (buf,0,256);
 if (CartName) strcpy (buf,CartName);
 if (fwrite(buf,1,256,f)!=256) return -2;
 for (i=0;i<4;++i)
 {
  memset (buf,0,256);
  if (DiskName[i]) strcpy (buf,DiskName[i]);
  if (fwrite(buf,1,256,f)!=256) return -2;
 }
 for (i=0;i<4;++i)
 {
  memset (buf,0,256);
  if (TapeName[i]) strcpy (buf,TapeName[i]);
  if (fwrite(buf,1,256,f)!=256) return -2;
 }
 Z80_GetRegs (&R);
...
int StartColeco(void)
{
 ...
 if (EmuMode)
 {
  ROMSize=8+8+32+32+32;                  /* OS-7, EOS, WP, CART, EXPROM                */
  ...
  if (ExpRomFile)
  {
   ...
   if (Verbose)
    if (j>1) printf("%d bytes loaded\n",j); else puts(P);
	 ...
	 if (Verbose) puts("Loading hard disk image:");
//	 HardDisk = fopen("~/hdisk.img","rw");
	 HardDisk = harddiskIdeCreate(0);
	 harddiskIdeReset(HardDisk);
	 printf("  Opening %s... ",HardDiskFile);
	 diskChange(0, HardDiskFile, 0);
	 if (HardDisk && diskPresent(0) && Verbose) puts(HardDisk ? "OK" : "FAILED");
